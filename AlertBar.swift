...
public enum AlertBarType {
    ...
    var backgroundColor: UIColor {
        switch self {
        case .success: return UIColor(0xFF0000)
...
public final class AlertBar {
    public static let alertshared = AlertBar()
    public static let kWindowLevel: CGFloat = UIWindowLevelStatusBar + 1
   // private
    //private
    internal var alertBarViews: [AlertBarView] = []
   // private
    public var alertoptions = Options(shouldConsiderSafeArea: true, isStretchable: false, textAlignment: .center)
    ...
    public func show(type: AlertBarType, message: String, duration: TimeInterval = 2, options: Options? = nil, autoDismiss: Bool, completion: (() -> Void)? = nil) {
        // Hide all before new one is shown.
        alertBarViews.forEach({ $0.hide() })

        let currentOptions = options ??  self.alertoptions//self.options
        ...
        alertBarView.show(duration: 2, translationY: -alertBarHeight, autoDismiss: autoDismiss) {
    ...
    public func show(error: Error, duration: TimeInterval = 2, options: Options? = nil, autoDismiss: Bool, completion: (() -> Void)? = nil) {
        let code = (error as NSError).code
        let localizedDescription = error.localizedDescription
        show(type: .error, message: "(\(code)) \(localizedDescription)", duration: duration, options: options, autoDismiss: autoDismiss,completion: completion)
    }
...
extension AlertBar: AlertBarViewDelegate {
    ...
    func alertBarViewHandleRotate(_ alertBarView: AlertBarView) {
     //Commented to remove change by rotation
...
public extension AlertBar {
    public static func setDefault(options: Options) {
       // alertshared.options = options
    alertshared.alertoptions = options
    }

    public static func show(type: AlertBarType, message: String, duration: TimeInterval = 2, options: Options? = nil, autoDismiss: Bool, completion: (() -> Void)? = nil) {
        alertshared.show(type: type, message: message, duration: duration, options: options, autoDismiss: autoDismiss,completion: completion)
    }

    public static func show(error: Error, duration: TimeInterval = 2, options: Options? = nil, autoDismiss: Bool, completion: (() -> Void)? = nil) {
        alertshared.show(error: error, duration: duration, options: options, autoDismiss: autoDismiss, completion: completion)
    }
...
public class AlertBarView: UIView {
    ...
    public static let kMargin: CGFloat = 2
    public static let kAnimationDuration: TimeInterval = 0.2
    ...
    func show(duration: TimeInterval, translationY: CGFloat, autoDismiss:Bool = true, completion: (() -> Void)?) {
        self.state = .showing
